/* Wave properties (STATIC VARIABLES) */
var	DENSITY = 0.75,
	FRICTION = 1.14,
	MOUSE_PULL = 0.07,
	AOE = 100,
	DETAIL = Math.round(WIDTH / 60),
	WATER_DENSITY = 1.07,
	AIR_DENSITY = 1.02,
	WAVE_HEIGHT_MULTIPLIER = 0.65;

function Wave () {
	this.mouseIsDown = false;
	this.ms = {
			x: 0,
			y: 0
		};

	this.mp = {
			x: 0,
			y: 0
		};

	this.particles = [];
}

/* Method Definitions */
Wave.prototype.initialize = function () {
	this.particles = [];

	// Generate wave particles
	for (var i = 0; i < DETAIL + 1; i++) {
		this.particles.push({
			x: WIDTH / (DETAIL - 4) * (i - 2),
			y: HEIGHT * 0.5,
			original: {
				x: 0,
				y: HEIGHT * WAVE_HEIGHT_MULTIPLIER
			},
			velocity: {
				x: 0,
				y: 1 + Math.random() * 3
			},
			force: {
				x: 0,
				y: 0
			},
			mass: 10
		});
	}

	// setInterval variables
	var twitchInterval1 = setInterval(this.twitch1.bind(this), 1500 + 300 * Math.sin(Math.random())),
		twitchInterval2 = setInterval(this.twitch2.bind(this), 1000 + 300 * Math.sin(Math.random())),
		twitchInterval3 = setInterval(this.twitch3.bind(this), 1300 + 700 * Math.sin(Math.random()));
};

Wave.prototype.twitch1 = function () {
	//this.ms.x = 0;
	//this.ms.y = 0;

	var forceRange = 3 + Math.random();
	this.insertImpulse(0.25 * WIDTH + 60 * Math.sin(2 * Math.PI * Math.random()) * Math.random(), 0.05 + (Math.random() * (forceRange * 2) - forceRange));
};

Wave.prototype.twitch2 = function () {
	//this.ms.x = 0;
	//this.ms.y = 0;

	var forceRange = 3 + Math.random();
	this.insertImpulse(0.5 * WIDTH + 100 * Math.cos(2 * Math.PI * Math.random()) * Math.random(), 0.08 + (Math.random() * (forceRange * 2) - forceRange));
};

Wave.prototype.twitch3 = function () {
	//this.ms.x = 0;
	//this.ms.y = 0;

	var forceRange = 3 + Math.random();
	this.insertImpulse(0.75 * WIDTH + 75 * Math.sin(2 * Math.PI * Math.random()) * Math.random(), 0.06 + (Math.random() * (forceRange * 2) - forceRange));
};

Wave.prototype.insertImpulse = function (positionX, forceY) {
	var particle = this.particles[Math.round(positionX / WIDTH * this.particles.length)];

	if (particle) {
		for (var i = 0; i < 1.5 * forceY; i++) {
			particle.force.y += 0.5;
		}
	}
};

Wave.prototype.drawWave = function () {
	var gradientFill = context.createLinearGradient(WIDTH * 0.5, HEIGHT * 0.2, WIDTH * 0.5, HEIGHT),
		len = this.particles.length,
		current,
		previous,
		next;
	
	gradientFill.addColorStop(0,'#012345');
	gradientFill.addColorStop(0.2,'#123456');
	gradientFill.addColorStop(0.4,'#345678');
	gradientFill.addColorStop(0.6,'#56789a');
	gradientFill.addColorStop(0.8,'#789abc');
	gradientFill.addColorStop(1,'#89abcd');

	context.fillStyle = gradientFill;
	context.beginPath();
	context.moveTo(this.particles[0].x, this.particles[0].y);

	for (var i = 0; i < len; i++) {
		current = this.particles[i];
		previous = this.particles[i - 1];
		next = this.particles[i + 1];

		if (previous && next) {
			var forceY = 0,
				prevY = current.y;

			forceY += -DENSITY * (previous.y - current.y);
			forceY += DENSITY * (current.y - next.y);
			forceY += DENSITY / 15 * (current.y - current.original.y);

			current.velocity.y += -(forceY / current.mass) + current.force.y;
			current.velocity.y /= FRICTION;
			current.force.y /= FRICTION;
			current.y += current.velocity.y;

			var distance = this.distanceBetween(this.mp, current);
			if (distance < AOE) {
				distance = this.distanceBetween(this.mp, {
					x: current.original.x,
					y: current.original.y
				});

				this.ms.x = this.ms.x * 0.98;
				this.ms.y = this.ms.y * 0.98;

				current.force.y += (MOUSE_PULL * (1 - (distance / AOE))) * this.ms.y;
			}

			//console.log(previous.y, current.y);
			context.quadraticCurveTo(previous.x, previous.y, previous.x + (current.x - previous.x) / 2, previous.y + (current.y - previous.y) / 2);
		}
	}

	context.lineTo(this.particles[this.particles.length - 1].x, this.particles[this.particles.length - 1].y);
	context.lineTo(WIDTH, HEIGHT);
	context.lineTo(0, HEIGHT);
	context.lineTo(this.particles[0].x, this.particles[0].y);
	context.fill();
};

Wave.prototype.mouseMove = function (e) {
	this.ms.x = Math.max(Math.min(e.layerX - this.mp.x, 15), -40);
	this.ms.y = Math.max(Math.min(e.layerY - this.mp.y, 15), -40);

	this.mp.x = e.layerX;
	this.mp.y = e.layerY;
};

Wave.prototype.mouseDown = function (e) {
	this.mouseIsDown = true;
};

Wave.prototype.mouseUp = function (e) {
	this.mouseIsDown = false;
};

Wave.prototype.distanceBetween = function (p1, p2) {
	var dx = p2.x - p1.x,
		dy = p2.y - p1.y;

	return Math.sqrt(dx * dx + dy * dy);
};

Wave.prototype.canvasResized = function () {
	console.log(1);
	for (var i = 0; i < DETAIL + 1; i++) {
		this.particles[i].x = WIDTH / (DETAIL - 4) * (i - 2);
		this.particles[i].y = HEIGHT * WAVE_HEIGHT_MULTIPLIER;

		this.particles[i].original.x = this.particles[i].x;
		this.particles[i].original.y = this.particles[i].y;
	}
};